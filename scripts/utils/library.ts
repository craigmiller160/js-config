...
export const isLibraryPresent = (
	cwdOrPackageJson: string | PackageJson,
	dependency: string | RegExp
): boolean => {
	...
	if (isPackageJson(cwdOrPackageJson)) {
		...
		packageJson = parsePackageJson(
			path.join(cwdOrPackageJson, 'package.json')
		);
	}
	...
	return func.pipe(
		...
		either.fold(
			(error) => {
				logger.error(
					`Error checking for the presence of dependency ${dependency} ${error}`
				);
				return false;
			},
			(result) => {
