...
const performPathCheck = (theDirectoryPath: string): CheckPathResult => {
	...
	logger.debug(
		`Performing path check. Path: ${theDirectoryPath} Result: ${result}`
	);
	return result;
};
...
export const checkPath = (
	theDirectoryPath: string,
	previousPathResults: ReadonlyArray<PathAndResult> = []
	...
	return match(result)
		...
		.with('js-config', () =>
			checkPath(path.join(theDirectoryPath, '..', '..'), newPathResults)
		)
		.with('node_modules', () =>
			checkPath(path.join(theDirectoryPath, '..'), newPathResults)
		)
		...
		.with('invalid', () =>
			either.left(new InvalidCwdPathError(newPathResults))
		)
		.exhaustive();
};
...
const handleError =
	...
	(error: Error): either.Either<Error, string> => {
		if (
			error instanceof InvalidCwdPathError &&
			error.pathResults.length === 2
		) {
			return either.right('');
		}
		...
		return either.left(
			new Error(
				`Error finding matching path for starting CWD: ${process.cwd()}`,
				{
					cause: error
				}
			)
...
export const findCwd = (
	...
	logger.info(
		`Attempting to find project CWD. Starting CWD: ${process.cwd()}`
	);
	return func.pipe(
		checkPath(process.cwd()),
		either.fold(handleError(process), (cwd) => {
