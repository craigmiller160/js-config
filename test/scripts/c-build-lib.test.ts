...
describe('c-build-lib', () => {
    it('runs build', () => {
        ...
        expect(runCommandSyncMock).toHaveBeenNthCalledWith(1, `${swcCommand} ${srcDir} -d ${esModuleDir} --config-file ${configPath} -C module.type=es6`);
        expect(runCommandSyncMock).toHaveBeenNthCalledWith(2, `${swcCommand} ${srcDir} -d ${commonjsDir} --config-file ${configPath} -C module.type=commonjs`);
        expect(runCommandSyncMock).toHaveBeenNthCalledWith(3, `${tscCommand} --declaration --emitDeclarationOnly --outDir ${typesDir}`);
    });
});